...
wget https://github.com/elseif/MikroTikPatch/releases/download/$ROS_VER/chr-$ROS_VER.img.zip -O chr.img.zip
...
do 
	...
	[ -n "${ADDR6[$i]}" -a -z "$VER_6" ] && echo "/ipv6 address add address=${ADDR6[$i]} interface=\$intfName advertise=no" >> /mnt/ros/rw/autorun.scr
done
